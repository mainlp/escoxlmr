...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(
            default=None,
            metadata={
                    "help": "The model checkpoint for weights initialization. Leave None if you want to train a model "
                            "from scratch."
                    },
            )
...
class DataCollatorForSpanLanguageModeling:
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # merge mask to generate mask span
        for i, line in enumerate(masked_indices):
            record_index = -1
            for j, value in enumerate(line):
                if value:
                    # value==True: this token is a mask
                    if record_index == -1:
                        # consider to merge this mask when record_index is in -1 status
                        if j != 0 and masked_indices[i][j - 1]:
                            # if previous one is also a mask, then do not consider to merge this mask and continue
                            continue
                        record_index = j
                    else:
                        if record_index == j - 1:
                            # if record index is the a previous token index, then do not consider to merge the
                            # previous mask and continue
                            record_index = -1
                            continue
                        # start to merge: remove the one mask token (under record index) and convert the previous
                        # token into a mask token
